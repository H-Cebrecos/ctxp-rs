...
fn text_encode_decode_roundtrip() {
    ...
    let mut buf = Vec::new();
    let mut enc = TextEncoder::new(&mut buf, &sources).unwrap();
    for event in &events {
...
fn binary_encode_decode_roundtrip() {
    ...
    let mut buf = Vec::new();
    let mut enc = BinaryEncoder::new(&mut buf, &sources).unwrap();
    for event in &events {
        enc.write_event(event).unwrap();
    }
    enc.flush().unwrap();
...
fn transcode_text_to_binary() {
    ...
    let mut enc = BinaryEncoder::new(&mut direct, &sources).unwrap();
    for event in &events {
        enc.write_event(event).unwrap();
    }
    enc.flush().unwrap();
    ...
    let mut bin_enc = BinaryEncoder::new(&mut transcoded, dec.sources()).unwrap();
    for event in dec {
...
fn transcode_binary_to_text() {
    ...
    let mut enc = TextEncoder::new(&mut direct_txt, &sources).unwrap();
    for event in &events {
        enc.write_event(event).unwrap();
    }
    enc.flush().unwrap();
    ...
    let mut bin_enc = BinaryEncoder::new(&mut bin_buf, &sources).unwrap();
    for event in &events {
    ...
    let mut txt_enc = TextEncoder::new(&mut transcoded_txt, dec.sources()).unwrap();
    for event in dec {
...
fn shared_encoder_matches_direct() {
    ...
    // direct encoding
    let mut direct = Vec::new();
    let mut enc = TextEncoder::new(&mut direct, &sources).unwrap();
    for event in &events {
        enc.write_event(event).unwrap();
    }
    enc.flush().unwrap();
